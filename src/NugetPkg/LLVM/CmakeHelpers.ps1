class CMakeConfig
{
    ...
    CMakeConfig([string]$platform, [string]$config, [string]$BaseGenerator, [string]$baseBuild, [string]$srcRoot)
    {
        ...
        $this.MsBuildCommandArgs = @("-m","-v:minimal")
...
function Build-Cmake([CMakeConfig]$config)
{
    ...
    $activity = "Building for {0}" -f($config.Name)
    Write-Information $activity

    $timer = [System.Diagnostics.Stopwatch]::StartNew()
    try
    {
        cmake --build $config.BuildRoot --config $config.ConfigurationType -- ($config.BuildCommandArgs -join ' ')
        if($LASTEXITCODE -ne 0 )
        {
